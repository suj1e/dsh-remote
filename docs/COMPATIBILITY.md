...
两个仓库的 `contract-v1.json` 与 wire-shape fixtures 已建立且一致；插件 Node 6 项、Swift XCTest 6 项、iOS 26 target build 通过。证据覆盖静态官方源码、DTO 和本地 multipart parser，不推出 HTTP adapter、WS mux、上传或任一 OS 已兼容。支持某一组合需要记录两个仓库 SHA、包/IPA 摘要、Xcode/iOS/LiveContainer 版本和三平台实际 Host 环境；表中目标版本不代表已经发布。
