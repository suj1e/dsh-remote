...
本轮增补（2026-09-26）：隔离实际 Host probe 已新增 `workspace/follow` baseline 和触发 `workspace/rename` 后的 upsert 验证；dsh-remote `pnpm test` 为 29 项通过。iOS 侧实现与对应 fixture/缓存/reducer 测试正在同一工作树推进，但这里记录的生产 Host 通过项仍只代表 macOS carrier，不代表 LiveContainer/iOS 联通或 Windows/Linux 支持；workspace-follow 断线换代恢复仍未验证。
